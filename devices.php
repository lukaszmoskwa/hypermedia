<?php
	...
	if (isset($_GET["id"])) { //da mettere in sicurezza!
		...
		query($sql);
		$sql = "SELECT D.ID, D.Category, D.Name, D.ImageURL, TechSpec, D.Description, D.Price, D.OldPrice, A.ID AS A_ID, A.Name AS A_Name, A.FAQ FROM Devices AS D INNER JOIN DevicesAssistance AS DA ON D.ID = DA.IDDevices INNER JOIN Assistance AS A ON DA.IDAssistance = A.ID WHERE D.ID=" . $_GET["id"];
		query($sql);
